      ...
      $.each(x.teams, function(j,team){
        if(teamIds.indexOf(team.id) == -1){
          ...
          league.teams[team.id] = {
            id: team.id,
            names: new Array(result.length),
            abbrevs: new Array(result.length),
            all_scores: new Array(result.length),
            all_ranks: new Array(result.length),
            trades:new Array(result.length),
            lineup_edits: new Array(result.length),
            moveToIR: new Array(result.length),
            all_acqs: new Array(result.length),
            draft_positions: new Array(result.length),
            all_rosters: new Array(result.length),
            owners: new Array(result.length),
            logos: new Array(result.length),
            wins: new Array(result.length),
            losses: new Array(result.length),
            ties: new Array(result.length),
            reg_wins: new Array(result.length).fill(0),
            reg_losses: new Array(result.length).fill(0),
            reg_ties: new Array(result.length).fill(0),
            playoff_wins: new Array(result.length).fill(0),
            playoff_losses: new Array(result.length).fill(0),
            playoff_ties: new Array(result.length).fill(0),
            championship_wins: new Array(result.length).fill(0),
            championship_losses: new Array(result.length).fill(0),
            championship_ties: new Array(result.length).fill(0),
            ptsFor: new Array(result.length),
            ptsAgainst: new Array(result.length),
            streakLength: new Array(result.length),
            streakType: new Array(result.length),
            winPct: new Array(result.length),
            szns: new Array(result.length)
          }
        }
        ...
        league.teams[team.id].all_ranks[i] = 
          {
            projected: team.draftDayProjectedRank,
            final: team.rankCalculatedFinal,
...
//MasterDataCompiler
function dataMod(teams, members, szns, stat){
  var teamNames = new Array(Object.keys(league.teams).length);
  ...
  var showPtsSel = (!showCumAvgSel && stat =="pts");
  var isFor = $('#pts-selector .secondary-select').eq(0).hasClass('secondary-selected');
  
  var showWinsSel = (!showCumAvgSel && (stat =="wins" || stat == "reg_wins" || stat == "playoff_wins" || stat == "championship_wins"));
  ...
  var showAcqsSel = (!showCumAvgSel && stat =="acqs");
  var isPicks = $('#acqs-selector .secondary-select').eq(0).hasClass('secondary-selected');
  
  var showRankSel = (!showCumAvgSel && stat =="rank");
    ...
    if(teams.indexOf(teamNames[team.id-1]) !== -1 || team.names.indexOf(teams[team.id-1]) !== -1){
      if(atLeastOneMember){
        ...
        if(stat == "pts"){
          ...
          avgProjRank = cumProjRank/szncounter;
          avgFinalRank = cumFinalRank/rankszncounter;
...
function getFilteredArrays(){
  ...
  var stat = $('#stat').val();
  dataMod(newTeams, newMembers, newSzns, stat);
...
$('#stat-selector').click(function(){
  ...
  if(clicked){
    $('#dropdown').slideUp(function(){
      $('.dropdown-item').show();
  $.each($('.dropdown-item'), function(i, el){
    if($(el).html() == $('#curr-selected-stat').html()){
      $(el).hide();
      return false;
    }
  })
...
$('.dropdown-item').click(function(){
  $('#curr-selected-stat').html($(this).html());
  $('#stat').prop('value',$(this).attr('data-val'));
  getFilteredArrays();
})
